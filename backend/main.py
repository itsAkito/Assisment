...
origins = [
    "http://localhost:3000", 
    "http://192.168.159.1:3000" # React app address
]
...
async def login(user_id: str = Form(...)):
    # In real apps, validate user_id + password here
    payload = {
        "user_id": user_id,
...
@app.get("/api/login-verify")
async def login_verify(credentials: HTTPAuthorizationCredentials = Depends(security)):
    token = credentials.credentials
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
        return {"status": "ok", "user_id": payload.get("user_id")}
